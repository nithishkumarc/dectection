...
while(True):
    ...
    if(score>15):
        for mobile in mobile_lst:
            message = "Please rescue your friend {frnd_name} and drop into this address {addr}".format(frnd_name=user_name,addr = address);
            x = datetime.datetime.now();
            kit.sendwhatmsg("+91"+mobile,message,int(x.strftime("%H")),int(x.strftime("%M"))+2,59);
            press('enter');
